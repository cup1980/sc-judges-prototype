...
# -*- coding: utf-8 -*-
"""
第3段（v8）：XserverのPDFから個別意見＋裁判官名簿を抽出する。
...
v6: PDF取得を自動リトライ化（ConnectTimeout等で落ちないよう指数バックオフ）。
v7: 古い判決の見出し癖に対応。
    P10 見出し内の改行/空白：「次⏎⏎のとおりである」「とおりで⏎ある」等を許容
        （TAIL を1文字ごとに空白許容で連結）。
    P9  ひらがな「つぎのとおりである」に対応。
    P8  意見と「は」の間の括弧注記「…意見（…）は、次のとおりである」を許容。
v8: P11 旧字/新字の混在を吸収（結語と本文で同一人物名の字体が違う件。
        例 奧野健一↔奥野健一）。ITAIJI に旧字体→新字体を追加。
    P12 「次のとおりである」の後ろの括弧注記
        「…意見は、次のとおりである（…追加意見がある。）。」を許容。
（v4までの確定事項）
...
def _ws(t):  # 各文字の間に \s* を挟み、改行/空白で割れても一致させる
    return r"\s*".join(re.escape(c) for c in t)
TAIL = r"(?:" + _ws("次のとおりである") + r"|" + _ws("つぎのとおりである") + r")"

# 本文の見出し「裁判官〔A、同B…〕の△△意見は〔、〕次のとおりである。」
#   ※ 読点(は の直後)は任意（古い判決は読点なし）
HEADING_RE = re.compile(
    r"裁判官\s*(" + NAMELIST + r")\s*の\s*(" + TYPE + r")\s*"
    r"(?:[（(][^）)]*[）)]\s*)?"           # 意見と「は」の間の括弧注記（任意）
    r"は[、，]?\s*" + TAIL +
    r"\s*(?:[（(][^）)]*[）)]\s*)?"          # 「とおりである」後ろの括弧注記（任意・P12）
    r"[。．]")
...
ITAIJI = str.maketrans({
    "邉":"辺","邊":"辺","惠":"恵","眞":"真","槇":"槙","德":"徳",
    # P11: 旧字体→新字体（人名で混在しやすいもの）
    "奧":"奥","郞":"郎","顯":"顕","﨑":"崎","濱":"浜","澤":"沢","齋":"斎","齊":"斉",
    "廣":"広","髙":"高","圓":"円","國":"国","莊":"荘","黑":"黒","對":"対","萬":"万",
    "與":"与","縣":"県","來":"来","瀨":"瀬","瀧":"滝","龜":"亀","寶":"宝","鐵":"鉄",
    "學":"学","廳":"庁","樂":"楽","拔":"抜","盜":"盗","顏":"顔","臺":"台","禮":"礼",
})
