...
"""
第4段（全件処理版）：判例をAI要約する。
- 入力: public/data/opinions_detail.json（第3段の出力。hasActiveJudge等を持つ）
...
- 分割実行: 既処理スキップ。1回 BATCH 件。

環境変数: ANTHROPIC_API_KEY, PDF_BASE, BATCH, SLEEP,
          MODEL_ACTIVE(既定 claude-sonnet-4-6), MODEL_OTHER(既定 claude-haiku-4-5)
...
# 要点抽出（stage4_gist相当を内蔵）
def normalize(t): return re.sub(r"-\s*\d+\s*-", "", t)
def extract_gist(text, max_chars=1800):
    text = normalize(text)
    cut = re.search(r"裁判官[^\sの、。]+?の(?:追加)?(?:反対意見|補足意見|意見)は、次のとおりである。", text)
    main = text[:cut.start()] if cut else text
    shubun = ""
    ms = re.search(r"主\s*文\s*(.{10,300}?)\s*理\s*由", text, re.S)
    if ms: shubun = re.sub(r"\s+"," ", ms.group(1)).strip()
    mj = re.search(r"(本件は、.{20,400}?事案である。)", main, re.S)
    jian = mj.group(1) if mj else ""
    mc = re.search(r"((?:以上によれば|よって、).{10,500}?(?:主文のとおり[^。]*。|破棄を免れない。|棄却すべきである。))", main, re.S)
...
def summarize(client, model, rec, gist):
    prompt = PROMPT.format(
        caseName=rec.get("caseName",""), court=rec.get("court",""),
        division=rec.get("division","") or "", judgeKind=rec.get("judgeKind",""),
        outcome=rec.get("outcome","") or "（記載なし）", field=rec.get("field","") or "（不明）",
        opinionsInfo=opinions_info(rec), gist=gist, tags="/".join(TAGS))
...
def main():
    ...
    print(f"全{len(detail)} / 済{len(done)} / 残{len(todo)} / 今回最大{BATCH}", file=sys.stderr)
    ...
    for rec in todo:
        ...
        try:
            r = requests.get(f"{PDF_BASE}hanrei-pdf-{hid}.pdf", headers=HEADERS, timeout=60)
            r.raise_for_status()
        ...
        if processed % 50 == 0:
            ...
            # 途中保存（タイムオーバー対策）
            json.dump(sorted(done.values(), key=lambda x:int(x["hanreiId"])),
    ...
    print(f"今回{processed}件 / 累計{len(done)} / 残り{remaining} を {OUT_JSON}", file=sys.stderr)
